    ...
    function getList(_type){
        $.post("//api.iotheme.cn/hot/get.php", { type: _type ,key:"<?php echo io_get_option('iowen_key') ?>" },function(data,status){ 
            ...
            for(let item of data) {
                html += '<div class="d-flex text-sm mb-2"><div><span class="hot-rank text-xs text-center">'+ (item['platform'])+'</span><a class="ml-2" href="'+item['link']+'" target="_blank" rel="external nofollow">'+item['title']+'</a></div><div class="ml-auto hot-heat d-none d-md-block">'+(item['price']).toFixed(1)+'</div></div>';
            } 
