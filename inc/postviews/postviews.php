		...
		if( is_single() || is_page() ) {
			...
			if( $should_count && ( ( isset( $views_options['use_ajax'] ) && (int) $views_options['use_ajax'] === 0 ) || ( !defined( 'WP_CACHE' ) || !WP_CACHE ) ) ) {
				if (io_get_option('views_r')>0) {
					...
					update_post_meta( $id, 'views', $view );
					do_action( 'postviews_increment_views', $view );
				} else {
...
function ioo_postview_cache_count_enqueue() {
	...
	if ( !wp_is_post_revision( $post ) && ( is_single() || is_page() ) ) {
		...
		if ( $should_count ) {
			wp_enqueue_script( 'wp-postviews-cache', get_theme_file_path('/inc/postviews/postviews-cache.js'), array(), '', true );
...
function the_views($display = true, $prefix = '', $postfix = '', $always = false) {
	...
	$views_options = get_option('views_options');
	if ($always || ioo_should_views_be_displayed($views_options)) {
		$output = $prefix.str_replace( array( '%VIEW_COUNT%', '%VIEW_COUNT_ROUNDED%' ), array( number_format_i18n( $post_views ), ioo_postviews_round_number( $post_views) ), stripslashes( $views_options['template'] ) ).$postfix;
		if($display) {
...
function ioo_views_options_parse($key) {
